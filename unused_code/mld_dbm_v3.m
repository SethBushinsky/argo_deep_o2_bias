...
% potential density
% 2019_02_08 SMB edit - the -0.2 deg C temperature threshold fails when
% using under ice data - at those temperatures, calculating a reference
% threshold that is colder actually gives reference water that is denser
% than the first point, automatically triggering the first point as the
% MLD, which causes an error.  I'm adding a flag to choose either T or
% density thresholds (DBM et al. 2004 give the option) to avoid this.  This
% also allows density as the MLD criteria in salinity stratified regions.
%
% 2021_08_23 changing the density criteria - now you pass through a
% threshold sigma change
function [mld_out,ild_out,sig_theta] = mld_dbm_v3(temp_cast,salin_cast,depth_cast,theta_yes, sig_theta_threshold_yes, sig_theta_threshold)
...
% reference depth is too deep - no shallow depths in cast
    ...
    if sig_theta_threshold_yes==1
        ref_sig_theta = sw_dens0(salin(ref_ind),theta(ref_ind)) + sig_theta_threshold;
    else
