...
glodap_offsets_filename = 'glodap_offsets_100km_10_to_2000_100m_0.005dens_0.005spice_5a.nc'
...
# +
depth_ranges = ['Deep', 'Shallow']

# DOXY_ADJUSTED_offset_trimmed_Shallow = []

for dep in depth_ranges:
    DOXY_ADJUSTED_offset_trimmed = []

    for n,g in offsets_g:
    
        if dep=='Deep':
            condition1 = g['PRES_ADJUSTED_float']>1450

        elif dep=='Shallow':
            condition1 = g['PRES_ADJUSTED_float']<200

        g_all = g.where(condition1, drop=False)
    
        # run a GESD test using "test_num" number of possible outliers
        test_num = int((len(g_all.DOXY_ADJUSTED_offset.dropna(dim="N_CROSSOVERS", how="any").values)*.1))
        ESD_test_out = ESD_Test(g_all.DOXY_ADJUSTED_offset.dropna(dim="N_CROSSOVERS", how="any").values, 0.05, test_num, False, True)


        # create temp_o2_offest to set all datapoints to nans that the GESD test says are outliers
        temp_o2_offset = g_all.DOXY_ADJUSTED_offset
    ...
    glodap_offsets['DOXY_ADJUSTED_offset_trimmed_' + dep]=result_da
...
condition1 = ~np.isnan(g.DOXY_ADJUSTED_offset_trimmed)
condition2 = g['PRES_ADJUSTED_float']>1700
g_deep = g.where(condition1 & condition2, drop=True)

condition3 =  g['PRES_ADJUSTED_float']<200
g_shallow = g.where(condition1 & condition3, drop=True)
...
var_hists = ['TEMP', 'PSAL', 'DOXY', 'NITRATE', 'pH_25C_TOTAL']
for n,g in offsets_g:
    print(n)
  
    plot_count = plot_count+1
  
    # now loop through two depth ranges: 
    condition1 = ~np.isnan(g.DOXY_ADJUSTED_offset_trimmed_Deep)
    condition2 = g['PRES_ADJUSTED_float']>1450
    g_deep_all = g.where(condition2, drop=True)
    g_deep_ox = g.where(condition1 & condition2, drop=True)

    condition4 = ~np.isnan(g.DOXY_ADJUSTED_offset_trimmed_Shallow)

    condition3 =  g['PRES_ADJUSTED_float']<250
    g_shallow_all = g.where(condition3, drop=True)
    g_shallow_ox = g.where(condition4 & condition3, drop=True)
    ...
    for dep in depth_ranges:
        ...
        if np.all(np.isnan(g_ox.DOXY_ADJUSTED_offset.values)):
            continue
            
        ncross = len(g_ox.DOXY_ADJUSTED_offset)

        #add crossover location map
        axn = plt.subplot(6,5,1+plot_start)
        print(ncross)
        
        axn.plot(g_ox.main_float_longitude,g_ox.main_float_latitude,'g+',markersize=10,label='Current float')
        #glodap
        glodap_lon = xr.where(g_ox.glodap_longitude>180,g_ox.glodap_longitude-360.,g_ox.glodap_longitude)
        axn.plot(glodap_lon,g_ox.glodap_latitude,'ro',label = 'Glodap',markersize=10)
        axn.legend()
        axn.set_title('WMO: %d; N: %d; Dist filt %d; Depth min %d max %d\nDepth filt %d; Dens %.4f; Spice %.4f' % 
                      (g_ox.main_float_wmo.values[0],ncross, glodap_offsets["dist"],
        ...
        #time
        axn = plt.subplot(6,5,(2+plot_start,4+plot_start))
        axn.plot(g_ox.main_float_juld,g_ox.DOXY_ADJUSTED_offset,'rx',label='float')
        axn.plot(g_ox.main_float_juld,g_ox['DOXY_ADJUSTED_offset_trimmed_' + dep],'ro',label='float_outliers_removed', markerfacecolor="None")
        axn.plot(g_ox.glodap_datetime,g_ox.DOXY_ADJUSTED_offset,'bx',label='GDAP')
        axn.plot(g_ox.glodap_datetime,g_ox['DOXY_ADJUSTED_offset_trimmed_' + dep],'bo', markerfacecolor="None")

        m1, b1 = np.polyfit(mdates.date2num(g_ox.main_float_juld),g_ox['DOXY_ADJUSTED_offset_trimmed_' + dep], 1)
        ...
        X_series = pd.Series(mdates.date2num(g_ox_sorted.glodap_datetime))
        Y_series = pd.Series(g_ox_sorted['DOXY_ADJUSTED_offset_trimmed_' + dep].values)
        CI_alpha = 0.95
        b_yx, a, r2, CI_alpha_slope, ttt, y_err = regress_confidence_sokal_rohlf(X_series, Y_series, CI_alpha)
        ...
        # extrapolate regression and CI if needed to intersect float_mid_date
        # should apply the slope of the 
        float_mid_date = mdates.date2num(g_all.main_float_juld.mean()) # mean float date in number
        if len(X_series)<5: # too short to realistically do a regression
            uncert_min = -100
            uncert_max = 100
        ...
        axn.legend()
        joined_dates = mdates.date2num(sorted(np.append(g_all.glodap_datetime.values, g_all.main_float_juld.values)))
        axn.set_xlim(mdates.num2date(joined_dates[0] - 365), mdates.num2date(joined_dates[-1] + 365))
        try:
            axn.set_ylim(min(g_all.DOXY_ADJUSTED_offset.values)  - 5,  max(g_all.DOXY_ADJUSTED_offset.values) + 5)
        except ValueError:
            pass

        axn.set_title('O2 Offsets vs date, Real ocean O2 change? ' + str(glodap_drift_possible))
    
        for idx, var in enumerate(var_hists):
            # Temperature
            axn = plt.subplot(6,5,5+plot_start+idx)
            g_plot = g_all[var + '_ADJUSTED_offset']
            g_mean = np.nanmean(g_plot.values).round(decimals=2)
            g_std = np.nanstd(g_plot.values).round(decimals=2)
           
            
            if var=='DOXY':
                g_plot_trim = g_ox['DOXY_ADJUSTED_offset_trimmed_' + dep]
                ...
                axn = plt.subplot(6,5,10+plot_start)
                #                 g_plot = g.DOXY_ADJUSTED_offset_trimmed
                g_mean = np.nanmean(g_plot_trim.values).round(decimals=2)
                g_std = np.nanstd(g_plot_trim.values).round(decimals=2)
                ...
                axn.set_title(var + ' %.2f +/- %.2f' % (g_mean,g_std))
                plt.grid()
    
            
         #O2 vs pressure
        axn = plt.subplot(6,5,11+plot_start)
        axn.plot(g_all.DOXY_ADJUSTED_offset,g_all.PRES_ADJUSTED_float,'bx')
        axn.plot(g_all['DOXY_ADJUSTED_offset_trimmed_' + dep], g_all.PRES_ADJUSTED_float,'bo', markerfacecolor="none")
        ...
        axn = plt.subplot(6,5,12+plot_start)
        axn.plot(g_all.DOXY_ADJUSTED_offset,g_all.PRES_ADJUSTED_glodap,'bx')
        axn.plot(g_all['DOXY_ADJUSTED_offset_trimmed_' + dep], g_all.PRES_ADJUSTED_glodap,'bo', markerfacecolor="none")
        ...
        #vs density
        axn = plt.subplot(6,5,13+plot_start)
        axn.plot(g_all.DOXY_ADJUSTED_offset,g_all.PDENS_float,'bx')
        axn.plot(g_all['DOXY_ADJUSTED_offset_trimmed_' + dep], g_all.PDENS_float,'bo', markerfacecolor="none")
        ...
        axn = plt.subplot(6,5,14+plot_start)
    #     axn.plot(g.DOXY_ADJUSTED_offset,g.DOXY_ADJUSTED_float,'bx')
        # trying lots of things:
#         float_time_days = mdates.date2num(g_all.main_float_juld)
#         time_since_deploy = float_time_days - float_time_days[0]

        obs_inds = np.unique(g_all.glodap_obs_index)
        obs_inds_scaled = np.zeros(len(g_all.glodap_obs_index))
        for idx, cr in enumerate(obs_inds):
            obs_inds_scaled[g_all.glodap_obs_index==obs_inds[idx]] = idx

        cn = axn.scatter(g_all['DOXY_ADJUSTED_offset_trimmed_' + dep], g_all.DOXY_ADJUSTED_float,30, obs_inds_scaled, 
...
#         cn = axn.scatter(g_all.DOXY_ADJUSTED_offset_trimmed, g_all.DOXY_ADJUSTED_glodap,30, obs_inds_scaled, 
#                     marker='o', cmap='tab20c')
#         plt.colorbar(cn,label='?')
...
#         plt.grid()
        ...
        smax = float(max(g_all.PSAL_ADJUSTED_float)) + 0.1
        smin = float(min(g_all.PSAL_ADJUSTED_float)) - 0.1
        tmax = float(max(g_all.TEMP_ADJUSTED_float)) + 0.1
        tmin = float(min(g_all.TEMP_ADJUSTED_float)) - 0.1
        ...
        axn = plt.subplot(6,5,15+plot_start)
        CS = plt.contour(si,ti,dens, linestyles='dashed', colors='k')
        axn.clabel(CS, fontsize=12, inline=1)
        axn.scatter(g_all.PSAL_ADJUSTED_float,g_all.TEMP_ADJUSTED_float,c=g_all['DOXY_ADJUSTED_offset_trimmed_' + dep])
        axn.set_xlim(min(g_all.PSAL_ADJUSTED_float) - .1, max(g_all.PSAL_ADJUSTED_float) + .1)
        axn.set_ylim(min(g_all.TEMP_ADJUSTED_float) - 0.1, max(g_all.TEMP_ADJUSTED_float) + 0.1)
    ...
    if stop_after_count>0:
        if plot_count==stop_after_count:
            break
...
#     axn.plot(g.DOXY_ADJUSTED_offset,g.DOXY_ADJUSTED_float,'bx')
    ...
    if stop_after_count>0:
        if plot_count==stop_after_count_count:
            break
